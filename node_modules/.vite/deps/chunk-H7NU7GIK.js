...
// node_modules/tegaki/dist/core-CIzLDu_Q.mjs
var defaultEffects = { pressureWidth: true };
var knownEffects = /* @__PURE__ */ new Set([
  "glow",
  "wobble",
  "pressureWidth",
  "taper",
...
function gradientColor(progress, colors, seed) {
  ...
  const scaledT = ((progress + seed * 0.1) % 1 + 1) % 1 * (colors.length - 1);
...
function drawGlyph(ctx, glyph, pos, localTime, lineCap, color, effects = [], seed = 0, getSubdivided, strokeEasing = defaultStrokeEasing) {
  ...
  const wobbleDy = (x, _y, idx) => {
    if (!hasWobble) return 0;
    if (wobbleMode === "noise") return wobbleAmplitude * (noise1d(x * 0.1 + idx * 0.5, seed * 1.3 + 1e3) * 2 - 1);
  ...
  for (const stroke of glyph.s) {
    ...
    if (rawPts.length === 1) {
      ...
      const baseLineWidth2 = Math.max(p[2], 0.5) * scale;
      let dotWidth = baseLineWidth2 + (Math.max(p[2], 0.5) * scale - baseLineWidth2) * pressureAmount;
    ...
    if (!needsPerSegment && !hasGradient) {
      ...
      for (let i = 1; i < tcount; i++) {
        ...
        if (needsPerSegment) {
          ...
          lw = Math.max(baseLineWidth + (perPoint - baseLineWidth) * pressureAmount, 0.5 * scale) * taperMultiplier(midProgress);
        }
        ctx.lineWidth = lw;
...
var COMPATIBLE_BUNDLE_VERSIONS = /* @__PURE__ */ new Set([0]);
var bundles = /* @__PURE__ */ new Map();
var warnedBundles = /* @__PURE__ */ new Set();
...
var CSS_TIME = "--tegaki-time";
var CSS_PROGRESS = "--tegaki-progress";
var CSS_DURATION = "--tegaki-duration";
var PADDING_H_EM = 0.2;
var MIN_LINE_HEIGHT_EM = 1.8;
var MIN_PADDING_V_EM = 0.2;
var cssPropertiesRegistered = false;
...
var PAD_V_CSS = "max(0.2em, 0.9em - 0.5lh)";
...
var TegakiEngine = class {
  ...
  */
  static renderElements(options, createElement2) {
    return {
      rootProps: buildRootProps(options),
      content: buildChildren(options, createElement2)
    };
  }
...
// node_modules/tegaki/dist/react-CVoAINjv.mjs
...
function TegakiRenderer(props) {
  const { as: Tag = "div", ref, font, text, children, time: timeProp, onComplete, effects, quality, timing, showOverlay, direction, editable, onTextChange, ...elementProps } = props;
  const containerRef = (0, import_react.useRef)(null);
  const engineRef = (0, import_react.useRef)(null);
  const resolvedText = text ?? coerceToString(children);
  const [internalText, setInternalText] = (0, import_react.useState)(resolvedText);
  const [prevResolvedText, setPrevResolvedText] = (0, import_react.useState)(resolvedText);
  ...
  const onTextChangeRef = (0, import_react.useRef)(onTextChange);
...
//# sourceMappingURL=chunk-H7NU7GIK.js.map
